...
def split_name(name):
   splited_str = name.split(maxsplit=1)
   name_dict = dict()
   name_dict['first_name'] = splited_str[0]
   name_dict['last_name'] = splited_str[1]
...
def build_list(item, instances=1):
    return [item for i in range(instances)]
